import {generateBootCamp,applyGates} from './engine.js';
export const SAVE_KEY='tiny-front-save';
const VERSION=3;
const stateFields=['phase','mission','score','missionStartScore','grenades','rockets','elapsed','kills','hutsDone','stars','bonus','evacReady','hutHint','camera','follow','bullets','bombs','decals','deaths','bootcamp'];
...
export function encodeSave(state,map,squad){
  return JSON.stringify({version:VERSION,seed:map.seed,
    state:{...pick(state,stateFields),selected:[...state.selected],bootcamp:!!state.bootcamp},
    ...
    gates:(map.gates||[]).map(g=>({x:g.x,y:g.y,cells:g.cells,open:!!g.open,id:g.id||0})),
    civilians:(map.civilians||[]).map(c=>({x:c.x,y:c.y,hp:c.hp,kind:c.kind,id:c.id,angle:c.angle||0,path:c.path||[],flee:c.flee||0})),
...
const recruit=value=>object(value,{id:integer,name:v=>typeof v==='string'&&v.length>0&&v.length<=24,rank:v=>integer(v)&&v<=7,dead:boolean})&&(value.medals===undefined||list(choice('star','cross','heart'),8)(value.medals));
...
export function decodeSave(raw,generateMap){
  ...
  if(!data||(data.version!==2&&data.version!==3)||!integer(data.seed))throw new Error('Unsupported save');
  if(data.version===2){
    data.version=3;data.state={...data.state,bootcamp:false};
    data.gates=data.gates||[];data.civilians=data.civilians||[];
    if(data.campaign?.roster)data.campaign.roster.forEach(r=>{r.medals=r.medals||[];});
  }
  const s=data.state;
  if(!object(s,{phase:choice('briefing','playing','paused','won','lost','over','map'),mission:v=>integer(v)&&v>=1,
    ...
    (s.bootcamp!==undefined&&!boolean(s.bootcamp))||
    ...
    !list(g=>object(g,{x:number,y:number,open:boolean,cells:list(c=>object(c,{x:integer,y:integer}),16)}))(data.gates||[])||
    !list(c=>object(c,{x:number,y:number,hp:number,kind:choice('villager','chicken')}))(data.civilians||[]))
    throw new Error('Invalid save');
  data.campaign.roster.forEach(r=>{r.medals=r.medals||[];});
  const map=s.bootcamp?generateBootCamp(data.seed):generateMap(data.seed,s.mission);
  ...
  const killed=data.enemies.filter(e=>e.hp<=0&&e.role!=='dummy').length;
  ...
  if(data.gates?.length){map.gates=data.gates;applyGates(map);}
  if(data.civilians)map.civilians=data.civilians.map((c,i)=>Object.assign({},map.civilians[i]||{maxHp:20,path:[],flee:0,cooldown:0},c));
  return {map,state:{...pick(s,stateFields),selected:new Set(s.selected),campaign:data.campaign,bootcamp:!!s.bootcamp},squad:data.squad.map(u=>pick(u,soldierFields))};
}
